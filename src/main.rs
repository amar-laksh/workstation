            ...
            loop {
                ...
                try!(face.detect_multi_scale(&reduced, &mut faces, 1.1, 2,
                    ...
                    if faces.len() != 0 {
                        ...
                        if area >= rect {
                            sh!("echo 0 > /sys/class/backlight/intel_backlight/brightness");
                            seen = 1;
                        } else if area < rect && seen == 1 {
                            sh!("echo 937 > /sys/class/backlight/intel_backlight/brightness");    
                        }
                    }
                    else {
                        ...
                        let mut elapsed;
                        let mut sec = 0;
                        let mut still_faces = ::opencv::types::VectorOfRect::new(); ;
                        let mut saved = 0;
                        while  sec <= timeout {
                            elapsed = now.elapsed();
                            sec = ((elapsed.as_secs() as f64) + (elapsed.subsec_nanos() as f64 / 1000_000_000.0)) as i64;
                            // println!("seconds elapsed:{:?}, timeout: {:?}", sec, timeout);
                            let mut frame = try!(core::Mat::new());
                            try!(cam.read(&mut frame));
                            if try!(frame.size()).width == 0 {
                                ::std::thread::sleep_ms(50);
                            ...
                            try!(imgproc::resize(&gray, &mut reduced, core::Size{width:0,height:0},
                                0.25f64, 0.25f64, imgproc::INTER_LINEAR));
                            still_faces = ::opencv::types::VectorOfRect::new();
                            try!(face.detect_multi_scale(&reduced, &mut still_faces, 1.1, 2,
                                objdetect::CV_HAAR_SCALE_IMAGE,
                                core::Size{ width:30, height:30 },
                                core::Size{ width:0, height:0 }));
...
fn main() {
    ...
    let matches = App::new("workstation")
        ...
        .about("helps you at the workstation by keeping you far from screen and locks system when you are away, among other things.")
        .arg(Arg::with_name("rect")
                 .required(true)
                 .takes_value(true)
                 .index(1)
                 .help("size of rectangle to trigger screen dimming"))
